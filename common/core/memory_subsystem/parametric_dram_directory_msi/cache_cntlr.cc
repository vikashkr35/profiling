   ...
   m_shmem_perf_model(shmem_perf_model)
{
  ...
  // registerStatsMetric(name, core_id, "Corresponding-prop-hit-L2", &stats.Corresponding_prop_hit_L2);
   ...
   registerStatsMetric(name, core_id, "num_struc_dram_time_add1", &stats.num_struc_dram_time_add1);
   registerStatsMetric(name, core_id, "num_struc_dram_time_add2", &stats.num_struc_dram_time_add2);
   //.................................................................................................
   for(CacheState::cstate_t state = CacheState::CSTATE_FIRST; state < CacheState::NUM_CSTATE_STATES; state = CacheState::cstate_t(int(state)+1)) {
      ...
      // ...................................neelam: CHANGE!!! from DROPLET.................................................................................
      registerStatsMetric(name, core_id, String("demand-stores-struc-misses")+CStateString(state), &stats.demand_stores_struc_misses_state[state]);
      ...
      bool count)
{
   ...
   bool prop = getMemoryManager()->isPropertyCacheline(ca_address);

   //assert to check structure data is read-only
   if (struc)
   {
      assert(mem_op_type != Core::WRITE);
   }
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   ...
   //.....................neelam: CHANGE!!! for cache bypassing .......................
   ...
   if (cache_hit)
   {
      if (isPrefetch == Prefetch::NONE && cache_block_info->hasOption(CacheBlockInfo::PREFETCH))
      {
         ...
         //........................... neelam: CHANGE!!! from DROPLET...............................
            if (struc)
               stats.hits_prefetch_struc++;
            else if (prop)
               stats.hits_prefetch_prop++;
         //.....................................................................
      ...
      /* TODO: if we end up getting the data from a sibling cache, the access time might be only that
         ...
         if (modeled)
         {
            ...
            SubsecondTime t_now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
            if (m_master->mshr.count(address)
               && (m_master->mshr[address].t_issue < t_now && m_master->mshr[address].t_complete > t_now) && (!struc))
            {
               SubsecondTime latency = m_master->mshr[address].t_complete - t_now;
               stats.mshr_latency += latency;
               getMemoryManager()->incrElapsedTime(latency, ShmemPerfModel::_USER_THREAD);
            ...
            else
            {
               if(!struc)
               {
                 ...
                 //add DRAM latency for structure data hit
                  SubsecondTime dram_latency_add = SubsecondTime::FS() * static_cast<uint64_t>(TimeConverter<float>::NStoFS(Sim()->getCfg()->getFloat("perf_model/dram/latency"))); // Operate in fs for higher precision before converting to uint64_t/SubsecondTime
                  getMemoryManager()->incrElapsedTime(dram_latency_add, ShmemPerfModel::_USER_THREAD);
                  // cout << dram_latency_add << endl;
                  stats.num_struc_dram_time_add1++; 
               }
            }
         }
      #endif
   ...
   if (cache_hit)
   {
      ...
      /* Store completion time so we can detect overlapping accesses */
      ...
      #else
         if (modeled && !first_hit && !m_passthrough && !struc)
         {
            ScopedLock sl(getLock());
            m_master->mshr[address] = make_mshr(t_issue, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD));
            cleanupMshr();
         ...
         {
            ScopedLock sl(getLock());
            m_master->mshr_struc[address] = make_mshr(t_issue, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD));
            cleanupMshrStruc();
         }
      #endif
   }
...
{
   //...................... neelam: CHANGE!!! added isStruc .......................
      bool isStruc = getMemoryManager()->isStructureCacheline(ca_address, m_cache_block_size);
   //..............................................................................
   ...
   switch (mem_op_type)
   {
      ...
      case Core::READ_EX:
         m_master->m_cache->accessSingleLine(ca_address + offset, Cache::LOAD, data_buf, data_length,
                                             getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD), update_replacement, isStruc);   //.... neelam: CHANGE!!! added isStruc ......
         break;
...
{
   bool isStruc = getMemoryManager()->isStructureCacheline(address, m_cache_block_size);  //.... neelam: CHANGE!!! added isStruc ......

   __attribute__((unused)) SharedCacheBlockInfo* cache_block_info = (SharedCacheBlockInfo*) m_master->m_cache->accessSingleLine(
      address, Cache::LOAD, data_buf, getCacheBlockSize(), getShmemPerfModel()->getElapsedTime(thread_num), update_replacement, isStruc);  //.... neelam: CHANGE!!! added isStruc ......
...
MYLOG("insertCacheBlock l%d @ %lx as %c (now %c)", m_mem_component, address, CStateString(cstate), CStateString(getCacheState(address)));
   ...
   m_master->m_cache->insertSingleLine(address, data_buf,
         ...
         getShmemPerfModel()->getElapsedTime(thread_num), this, isStruc);   //.... neelam: CHANGE!!! added isStruc ......
...
assert(data_length==getCacheBlockSize());
      ...
      __attribute__((unused)) SharedCacheBlockInfo* cache_block_info = (SharedCacheBlockInfo*) m_master->m_cache->accessSingleLine(
         address + offset, Cache::STORE, data_buf, data_length, getShmemPerfModel()->getElapsedTime(thread_num), false, isStruc);  //.... neelam: CHANGE!!! added isStruc ......
...
MYLOG("wakeup user #%u", request->cache_cntlr->m_core_id);
         ...
         #else
         {
            if(!getMemoryManager()->isStructureCacheline(address, m_cache_block_size))
            {
               {
                  ScopedLock sl(request->cache_cntlr->getLock());
                  request->cache_cntlr->m_master->mshr[address] = make_mshr(request->t_issue, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD));
               ...
               {
                  ScopedLock sl(request->cache_cntlr->getLock());
                  request->cache_cntlr->m_master->mshr_struc[address] = make_mshr(request->t_issue, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD));
                  cleanupMshrStruc();
               }
            }
         }
...
CacheCntlr::cleanupMshr()
{
   ...
   //.................. neelam: CHANGE!!! added code to cleanupMSHR() for struc data entries...............
   void
   CacheCntlr::cleanupMshrStruc()
   {
      /* Keep only last 128 MSHR entries for structure data */
      while (m_master->mshr_struc.size() > 128)
      { // > commit_width is kept
         IntPtr address_min = 0;
         SubsecondTime time_min = SubsecondTime::MaxTime();
         for (Mshr::iterator it = m_master->mshr_struc.begin(); it != m_master->mshr_struc.end(); ++it)
         {
            if (it->second.t_complete < time_min)
            {
               address_min = it->first;
         ...
         m_master->mshr_struc.erase(address_min);
      }
   }
