...
class CacheSet
{
   ...
   public:
      ...
      void read_line(UInt32 line_index, UInt32 offset, Byte *out_buff, UInt32 bytes, bool update_replacement, bool isStruc = false);
      void write_line(UInt32 line_index, UInt32 offset, Byte *in_buff, UInt32 bytes, bool update_replacement, bool isStruc = false);
      CacheBlockInfo* find(IntPtr tag, UInt32* line_index = NULL);
      bool invalidate(IntPtr& tag);
      void insert(CacheBlockInfo* cache_block_info, Byte* fill_buff, bool* eviction, CacheBlockInfo* evict_block_info, Byte* evict_buff, CacheCntlr *cntlr = NULL, bool isStruc = false, UInt32 set_index = 0, UInt64 num_accesses = 0); //.... neelam: CHANGE!!! added isStruc for cache policy separate ways; set_index and num_accesses for DRRIP
      ...
      virtual UInt32 getReplacementIndex(CacheCntlr *cntlr, bool isStruc = false, UInt32 set_index = 0, UInt64 num_accesses = 0) = 0; //......... neelam: CHANGE!!! added isStruc for cache bypassing; Added set_index, num_accesses for DRRIP
      virtual void updateReplacementIndex(UInt32 accesses_index, bool isStruc = false) = 0; //......... neelam: CHANGE!!! for cache bypassing
