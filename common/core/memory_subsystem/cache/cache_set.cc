...
#include "cache_set_lru_s_ns.h" //........ neelam: CHANGE!!! for cache bypassing
#include "cache_set_drrip_s_ns.h" //neelam: CHANGE!!! for DRRIP at LLC with separate ways allocation to structure and non-structure data
...
void
CacheSet::read_line(UInt32 line_index, UInt32 offset, Byte *out_buff, UInt32 bytes, bool update_replacement, bool isStruc)
{
   ...
   if (update_replacement)
      updateReplacementIndex(line_index, isStruc); //.... neelam: CHANGE!!! added isStruc ......
}

void
CacheSet::write_line(UInt32 line_index, UInt32 offset, Byte *in_buff, UInt32 bytes, bool update_replacement, bool isStruc)
{
   assert(offset + bytes <= m_blocksize);
...
void
CacheSet::insert(CacheBlockInfo* cache_block_info, Byte* fill_buff, bool* eviction, CacheBlockInfo* evict_block_info, Byte* evict_buff, CacheCntlr *cntlr, bool isStruc, UInt32 set_index, UInt64 num_accesses) //.... neelam: CHANGE!!! added isStruc ......
{
   ...
   const UInt32 index = getReplacementIndex(cntlr, isStruc, set_index, num_accesses);
      ...
      UInt32 associativity, UInt32 blocksize, CacheSetInfo* set_info)
{
   ...
   switch(policy)
   {
      ...
      //............... neelam: CHANGE!!! Added new cache policy to implement separate ways for structure data ..........
      //LRU
      case CacheBase::LRU_S_NS:
         return new CacheSetLRU_S_NS(cache_type, associativity, blocksize, dynamic_cast<CacheSetLRU_S_NS_Info*>(set_info), getNumQBSAttempts(policy, cfgname, core_id));
      //..................................................................
      ...
      case CacheBase::DRRIP_S_NS: //..... neelam: CHANGE!!! added DRRIP_S_NS cache policy
...
CacheSet::createCacheSetInfo(String name, String cfgname, core_id_t core_id, String replacement_policy, UInt32 associativity)
{
   ...
   switch(policy)
   {
      ...
      case CacheBase::SRRIP:
      case CacheBase::SRRIP_QBS:
      //............... neelam: CHANGE!!! Added new cache policy to place structure data into the separate 8 ways  of a cache ..........
      case CacheBase::DRRIP_S_NS:
      //................................................................................................................................
         return new CacheSetInfoLRU(name, cfgname, core_id, associativity, getNumQBSAttempts(policy, cfgname, core_id));
      //............... neelam: CHANGE!!! Added new cache policy to place structure data into the separate 8 ways  of a cache ..........
      //LRU
      case CacheBase::LRU_S_NS:
         return new CacheSetLRU_S_NS_Info(name, cfgname, core_id, associativity, getNumQBSAttempts(policy, cfgname, core_id));
      //................................................................................................................................
...
CacheSet::parsePolicyType(String policy)
{
   ...
   if (policy == "lru_s_ns")
      return CacheBase::LRU_S_NS;
   if (policy == "drrip_s_ns")
      return CacheBase::DRRIP_S_NS;
   //if (policy == "lru_s_ns_qbs")
     // return CacheBase::LRU_S_NS_QBS;
